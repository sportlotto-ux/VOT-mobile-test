...
class Xtags {
    ...
    constructor(xtags: String) {
        val parsed = XTags.parseFrom(Base64.decode(xtags, Base64.URL_SAFE))
        values = parsed?.xtagsList?.associate { it.key to it.value }.orEmpty()
    }
    ...
    /**
     *  Whether the format uses dynamic range compression.
     */
