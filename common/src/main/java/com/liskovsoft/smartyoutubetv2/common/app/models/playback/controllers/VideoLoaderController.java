...
public class VideoLoaderController extends BasePlayerController {
    ...
    public boolean canReopenWithTranslation() {
        ...
        return (mLastFormatInfo != null && effectiveOpenTypeForTranslation() != 0) || (player != null && player.containsMedia());
    }
    public com.liskovsoft.mediaserviceinterfaces.data.MediaItemFormatInfo getLastFormatInfo() {
        return mLastFormatInfo;
    }
    private int effectiveOpenTypeForTranslation() {
        if (mLastFormatInfo == null) return 0;
        if (mLastFormatInfo.containsDashFormats()) return 1;
        if (mLastFormatInfo.containsSabrFormats()) return 2;
        if (mLastFormatInfo.getAdaptiveFormats() != null && !mLastFormatInfo.getAdaptiveFormats().isEmpty()) return 1;
        return 2;
    }
